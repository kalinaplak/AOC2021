...
let getColumn =
    fun (array: array<list<char>>) (index: int) ->
        array
...
let binaryStringToInt =
    fun input -> System.Convert.ToInt32(input |> List.fold (+) "", 2)
...
let calcEpsilon =
    fun bits -> calcGamma bits |> reverseBitList

let rec calcRating =
    fun pos (bits: array<list<char>>) factorFn ->
